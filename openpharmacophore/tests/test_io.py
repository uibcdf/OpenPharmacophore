...
def test_from_pharmer():
    ...
    points, molecular_system, ligand = from_pharmer("./openpharmacophore/data/pharmacophores/pharmer/1M70.json", 
                                            load_mol_sys=False)
    assert len(points) == 5
...
def test_read_pharmagist():
    ...
    pharmacophores = read_pharmagist(streptadivin_file)
    assert len(pharmacophores) == 6
    assert len(pharmacophores[0]) == 9
    assert len(pharmacophores[1]) == 10
    assert len(pharmacophores[2]) == 16
    assert len(pharmacophores[3]) == 11
    assert len(pharmacophores[4]) == 13
    assert len(pharmacophores[5]) == 13
    ...
    assert len(pharmacophores[1]) == 21
    assert len(pharmacophores[2]) == 16
    assert len(pharmacophores[3]) == 18
    assert len(pharmacophores[4]) == 10
    assert len(pharmacophores[5]) == 12
    assert len(pharmacophores[6]) == 11
    assert len(pharmacophores[7]) == 14
...
def test_to_ligandscout(three_element_pharmacophore, five_element_pharmacophore):
    ...
    # Test for three element pharmaocphore
